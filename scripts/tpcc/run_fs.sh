...
run_workload()
{
    ...
    if [ $run_boost -eq 1 ]; then
        export LD_LIBRARY_PATH=$src_dir/splitfs-so/tpcc/$mode
    ...
    if [ $run_boost -eq 1 ]; then
        time LD_PRELOAD=$src_dir/splitfs-so/tpcc/$mode/libnvp.so $tpcc_dir/tpcc_start -w 4 -c 1 -t 200000 2>&1 | tee $fs_results/run$run_id
    else
